...
fn main() {
    ...
    fn run_c(n: usize) -> (f64, Vec<f64>) {
        unsafe {
            ...
            for i in 0..n { *x.add(i) = init[i]; }
            let mut fx = 0.0f64;
            c_ffi::lbfgs(n as c_int, x, &mut fx, Some(c_rosenbrock_nd), None,
                std::ptr::null_mut(), std::ptr::null_mut());
            let mut result = vec![0.0; n];
            for i in 0..n { result[i] = *x.add(i); }
            c_ffi::lbfgs_free(x);
            (fx, result)
        }
    }
    ...
    for &n in &[10, 100, 1000, 10000] {
        ...
        let rel_err_fx = if cfx != 0.0 { abs_err_fx / cfx.abs() } else { abs_err_fx };
        ...
        println!("{:>8} {:>16.6e} {:>16.6e} {:>14.2e} {:>14.2e}",
            n, rfx, cfx, abs_err_fx, rel_err_fx);
        println!("{:>8} {:>33} {:>14.2e} {:>14.2e}",
